...
class Facebook implements ProviderInterface
{
    ...
    {
        if ($token = $this->rlDb->getOne('Token', "User_ID = 1", 'autoposter_tokens')) {
            return $token;
        }
    ...
    public function getPageToken()
    {
        ...
        return $this->rlDb->getOne('Page_token', "User_ID = 1", 'autoposter_tokens');
    }
    ...
    {
        return $this->rlDb->getOne('Token', "`User_ID` = {$user_id}", 'autoposter_tokens') ? true : false;
    }
    ...
    public function tokenExistingCheck($type, $user_id)
    {
        ...
        return $this->rlDb->getOne($find, "`User_ID` = {$user_id}", 'autoposter_tokens') ? true : false;
    }
    ...
    /**
     * Fetch neccessary debugging data of the current token
     *
     * @return array - token debuging data
     */
    ...
    public function removeToken($admin_id)
    {
        if ($this->isTokenExist($admin_id)) {
            $token = $this->getToken();
            $sql = "DELETE FROM `" . RL_DBPREFIX . "autoposter_tokens` WHERE `Token` = '{$token}' ";
            $this->rlDb->query($sql);
        }
    }
