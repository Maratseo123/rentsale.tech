...
class rlReportBrokenListing
{
    ...
    public function hookStaticDataRegister()
    {
        ...
        $rlStatic->addFooterCSS($this->configs['url']['static']['front'] . 'style.css', $in_pages, true);
        ...
        switch ($item) {
            case 'RBLAddReportItem':
                ...
                if ($reportPoint->onSubmit($reports)) {
                    $out = $ajaxWrapper->throwSuccessMessage($this->flLangs['rbl_points_added']);
                }
                break;
            ...
            case 'RBLGetPointsByKey':
                ...
                if ($reportPoints = $reportPoint->getPointByKey($key)) {
                    ...
                    $out = $ajaxWrapper->throwErrorMessage($this->flLangs['rbl_something_wrong']);
                }
                break;
            case 'RBLEditReportItem':
                ...
                if ($reportPoint->edit($new_data, $key)) {
                    $out = $ajaxWrapper->throwSuccessMessage($this->flLangs['rbl_point_changed']);
                } else {
                    $out = $ajaxWrapper->throwErrorMessage($this->flLangs['rbl_something_wrong']);
                }
                break;
            case 'RBLDeleteReportPoint':
                ...
                if (!$reports_exists) {
                    if ($reportPoint->deleteByKey($key)) {
                        $out = $ajaxWrapper->throwSuccessMessage($this->flLangs['rbl_point_removed']);
                    } else {
                        $out = $ajaxWrapper->throwErrorMessage($this->flLangs['rbl_something_wrong']);
                    }
                    return;
            ...
            case 'RBLDeleteReport':
                ...
                if ($reportDetail->onReportDelete(request('id'), request('reason'))) {
                    $out = $ajaxWrapper->throwSuccessMessage($this->flLangs['reportbroken_listing_has_been_added']);
                } else {
                    $out = $ajaxWrapper->throwErrorMessage($this->flLangs['rbl_something_wrong']);
                }
                break;
            case 'RBLRemoveAllReportsOfTheListing':
                ...
                if($this->removeAllReportsByListing(request('id'))) {
                    $out = $ajaxWrapper->throwSuccessMessage($this->flLangs['rbl_reports_removed']);
                }
                break;
            case 'RBLRemoveListing':
                if ($this->removeListing(request('id'))) {
                    FlynaxObjectsContainer::getObject('rlNotice')
                        ->saveNotice($this->flLangs['rbl_listing_has_been_removed']);
        ...
        switch ($request_mode) {
            case 'RBLAddReport':
                ...
                if (!$this->validateIP($ip, $data['Listing_ID'])) {
                    $out = $ajaxWrapper->throwErrorMessage($this->flLangs['rbl_ip_wrong']);
                    return false;
                }
                
                if ($id = $report->add($data)) {
                    $out = $ajaxWrapper->throwSuccessMessage($this->flLangs['rbl_report_added'], $id);
                } else {
                    $out = $ajaxWrapper->throwErrorMessage($this->flLangs['rbl_something_wrong']);
                }
                break;
            case 'RBLRemoveReport':
                ...
                if ($report->delete($report_id)) {
                    $out = $ajaxWrapper->throwSuccessMessage($this->flLangs['rbl_report_removed']);
                } else {
                    $out = $ajaxWrapper->throwErrorMessage($this->flLangs['rbl_something_wrong']);
                }
                break;
