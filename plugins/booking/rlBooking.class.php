...
//                            : $currentBookedTime;
    ...
    public function ajaxSaveRateRange($listing_id, $rate_range)
    {
        ...
        if ($rlActions->insertOne($insert, 'booking_rate_range')) {
            // add description
            if ($adapt_range[3]) {
                ...
                if ($insert_lang_keys) {
                    $rlActions->insert($insert_lang_keys, 'lang_keys');
                }
            }
    ...
    {
        global $listing_id, $languages, $rlDb;

        $listingID = (int) $listing_id;
        if (!$listingID && $instance && $instance->listingID) {
    ...
    public function prepareBookingOrder()
    {
        ...
        if ($bookingData['listing_id']
            ...
            if (!$listing_data['listing_link']) {
                $listing_data['url'] = $reefless->getListingUrl($listing_data);
            }
            ...
            $bread_crumbs[1] = array(
                'name' => $listing_data['listing_title'],
                'path' => str_replace(
                    $config['mod_rewrite'] ? '.html' : 'index.php?page=',
                    '',
                    str_replace(RL_URL_HOME, '', $listing_data['listing_link'])
                ),
            );

            $bread_crumbs[] = array('name' => $lang['pages+name+booking_order']);
    ...
    public function bookingOrder($bookingData = array())
    {
        ...
        if ($config['booking_notify_admin_by_email'] || $config['booking_notify_email']) {
            ...
            $currency               = $lang['data_formats+name+' . $currency];

            $price = $config['system_currency_position'] == 'before'
            ? ($currency ?: $config['system_currency']) . ' ' . $amount
            : $amount . ' ' . ($currency ?: $config['system_currency']);
            ...
            if ($configs['Prepayment']) {
                ...
                $rest = $rlValid->str2money(
                    round(
                        (floatval(
                            str_replace(',', '', $bookingData['total_cost']))
                        ) - floatval(
                            str_replace(',', '', $bookingData['prepayment'])
                        ),
                        2
                    )
                );
    ...
    public function initPrepayment()
    {
        ...
        if (!$rlPayment->isPrepare()) {
            ...
            ? round((floatval(str_replace(',', '', $booking_data['total_cost'])) / 100) * $configs['Prepayment'], 2)
            : 0;
...
HTML;
    ...
    public function _deleteListingData($id)
    {
        ...
        $rlDb->query("DELETE FROM `{db_prefix}lang_keys` WHERE `Key` = 'booking_range+regular+desc+{$id}'");
    ...
    {
        global $rlActions, $lang, $languages, $rlDb;
        ...
        if ($listing_id && $rate_info) {
            ...
            if ($update_lang_keys) {
                $rlActions->update($update_lang_keys, 'lang_keys');
            }
    ...
    public function ajaxCaseBookingSaveConfigs($item)
    {
        ...
        if ($listing_id && $formData) {
            ...
            if ($GLOBALS['rlDb']->getOne('ID', "`Listing_ID` = {$listing_id}", 'booking_configs')) {
                $update = array(
                    'fields' => array(
                ...
                $rlActions->updateOne($update, 'booking_configs');
            } else {
                $insert = array(
                    'Listing_ID'          => $listing_id,
                    'Deny_guest'          => $data['deny_guest'],
                ...
                $rlActions->insertOne($insert, 'booking_configs');
            }
    ...
    public function convertPricesInRates(&$bookingRates = [], $isEscort = false)
    {
        ...
        $systemCurrency = $rlCurrencyConverter->systemCurrency[$config['system_currency_code']];

        // Add missing rates, because hook "specialBlock" did not run yet
        if ($rlCurrencyConverter && !$rlCurrencyConverter->rates) {
        ...
        foreach ($bookingRates as &$rate) {
            if ($isEscort) {
                ...
                if (isset($subtitle[0]) && isset($subtitle[1])) {
                    ...
                    foreach ($rlCurrencyConverter->rates as $converterRateKey => $converterRate) {
                    ...
                    if ($rateCurrency != $systemCurrency) {
                        $ratePrice = $ratePrice / $rlCurrencyConverter->rates[$rateCurrency]['Rate'];
                        $ratePrice = explode('.', $ratePrice);
                        $ratePrice = $ratePrice[0] . '.' . substr($ratePrice[1], 0, 2);

                        if ($config['system_currency_position'] === 'before') {
                            $rate['subtitle'] = $rlCurrencyConverter->rates[$systemCurrency]['Symbol']
                                . ' '
                                . $ratePrice;
                        } else {
                            $rate['subtitle'] = $ratePrice
                                . ' '
                                . $rlCurrencyConverter->rates[$systemCurrency]['Symbol'];
                        }
                    }
                }
            } else {
                ...
                if ($rlCurrencyConverter && $rateCurrency != $systemCurrency) {
                    $ratePrice = $ratePrice / $rlCurrencyConverter->rates[$rateCurrency]['Rate'];
                    $ratePrice = explode('.', $ratePrice);
                    $ratePrice = $ratePrice[0] . '.' . substr($ratePrice[1], 0, 2);

                    $rateCurrency = $systemCurrency;
                }
            }
        }
    }
