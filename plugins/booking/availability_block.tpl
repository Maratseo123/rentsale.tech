                    ...
                    {foreach from=$listing_types item='booking_ltype'}
                        {if $booking_ltype.Key && $booking_ltype.Booking_type != 'none'}
                            <option value="{$booking_ltype.Key}"
                                data-path="{$pages[$booking_ltype.Page_key]}"
                                data-type="{$booking_ltype.Booking_type}">{$booking_ltype.name}</option>
                        {/if}
                    {/foreach}
                {/if}
            </select>
            ...
            if (!$checkInField.val() || !$checkOutField.val() || !$listingType.val()) {
                ...
                if (action_url && ltype_path) {
                    if (rlConfig['mod_rewrite']) {
                        action_url += ltype_path + '/' + '{/literal}{$search_results_url}{literal}.html';
                    } else {
                        action_url += '?page=' + ltype_path + '&' + '{/literal}{$search_results_url}{literal}';
                    }
