        ...
        {foreach from=$requests item='request' key='rKey'}
            <div class="row" id="item_request_{$request.ID}">
                ...
                <div data-caption="{$lang.listing}" class="content">
                    <a href="{$rlBase}{if $config.mod_rewrite}{$pages.booking_requests}/{str2path string=$request.title}-r{$rKey}.html{else}?page={$pages.booking_requests}&id={$rKey}{/if}" title="{$lang.booking_page_details}">{$request.title}</a>
                </div>
