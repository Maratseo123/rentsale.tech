...
{if $isLogin}
    <div class="content-padding {if defined('IS_ESCORT')}reservations-escort{/if}">
        {if $reservations}
            ...
            {math assign='prev_page' equation='current - 1' current=$pInfo.current}

            {if $config.mod_rewrite}
                {assign var="prev_page" value='/index'|cat:$prev_page|cat:'.html'}
            {else}
                {assign var="prev_page" value='&pg='|cat:$prev_page}
            {/if}
            ...
            {* @todo 3.1.0 - Remove this when "compatible" will be more than 4.6.2 *}
            {if $config.rl_version|version_compare:'4.6.2' <= 0}flynaxTpl.hisrc();{/if}
            ...
            {if $config.mod_rewrite}
                {if $pInfo.current > 2}
                    prev_page = '{$booking_reservations_url|replace:".html":$prev_page}';
                {else}
                    prev_page = '{$booking_reservations_url}';
                {/if}
            {else}
                {if $pInfo.current > 2}
                    prev_page = '{$booking_reservations_url|cat:$prev_page}';
                {else}
                    prev_page = '{$booking_reservations_url}';
                {/if}
            {/if}
