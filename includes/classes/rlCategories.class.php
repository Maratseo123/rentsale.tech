...
class rlCategories
{
    ...
    public function categoryWalker($category_id = false, $mode = false, $data = array(), $new_id = false, $initial_category = false)
    {
        ...
        switch ($mode) {
            ...
            case 'delete':
                /* get child categories */
                $child = $rlDb->fetch(array('ID', 'Parent_ID', 'Key', 'Type'), array('Parent_ID' => $category_id));
                ...
                if (!empty($child)) {
                    foreach ($child as $key => $value) {
                        /* delete categories */
                        $rlDb->query("DELETE FROM `{db_prefix}categories` WHERE `ID` = '{$child[$key]['ID']}' LIMIT 1");
                        Category::deleteRelatedPhrases($child[$key]['Key']);
            ...
            case 'trash':
                /* get child categories */
                $child = $rlDb->fetch(array('ID', 'Parent_ID', 'Key'), array('Parent_ID' => $category_id));
                $listings = $rlDb->fetch(array('ID`, UNIX_TIMESTAMP(`Pay_date`) AS `Pay_date', 'Category_ID', 'Account_ID'), array('Category_ID' => $category_id), null, null, 'listings');
                ...
                if (!empty($child)) {
                    foreach ($child as $key => $value) {
                        /* trash categories */
                        $rlDb->query("UPDATE `{db_prefix}categories` SET `Status` = 'trash' WHERE `ID` = '{$child[$key]['ID']}' LIMIT 1");

                        $this->categoryWalker($child[$key]['ID'], 'trash', $data);
                    }
                }
                break;
            ...
            case 'restore':
                /* get child categories */
                $child = $rlDb->fetch(array('ID', 'Parent_ID', 'Key'), array('Parent_ID' => $category_id));
                $listings = $rlDb->fetch(array('ID`, UNIX_TIMESTAMP(`Pay_date`) AS `Pay_date', 'Category_ID'), array('Category_ID' => $category_id), null, null, 'listings');
                ...
                if (!empty($child)) {
                    foreach ($child as $key => $value) {
                        /* restore categories */
                        $rlDb->query("UPDATE `{db_prefix}categories` SET `Status` = 'active' WHERE `ID` = '{$child[$key]['ID']}' LIMIT 1");

                        $this->categoryWalker($child[$key]['ID'], 'restore', $data);
                    }
                }
                break;
    ...
    public function ajaxDeleteCategory($key = false, $replace = false, $direct = false)
    {
        ...
        Category::deleteRelatedPhrases($key);
    ...
    {
        global $_response, $lang, $config, $rlCache, $rlDb;
        ...
        if (in_array($action, ['activate', 'approve'])) {
            ...
            foreach ($ids as $id) {
                ...
                $GLOBALS['rlListings']->listingStatusControl(['Category_ID' => $id], $status);
            }
            ...
            foreach ($ids as $id) {
                ...
                $this->categoryWalker($id, $config['trash'] ? 'trash' : 'delete');
                $this->deleteCatRelations($id);
                Category::resetGeneralCategory($id, $categoryInfo['Type']);
                Category::deleteRelatedPhrases($categoryInfo['Key']);
            }
