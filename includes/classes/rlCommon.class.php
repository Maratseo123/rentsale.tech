...
class rlCommon
{
    ...
    public function simulateCatBlocks()
    {
        ...
        foreach ($cat_blocks as $types) {
            if ($GLOBALS['tpl_settings']['combine_categories_boxes']) {
                if ($types[0] && $blocks['ltcb_' . $types[0]]) {
                    $blocks['ltcb_' . $types[0]]['Content'] = '{include file="blocks/categories_block.tpl" types="' . implode(',', $types) . '"}';
                }
                ...
                foreach ($types as $type) {
                    if ($blocks['ltcb_' . $type]) {
                         $blocks['ltcb_' . $type]['Content'] = '{include file="blocks/categories_block.tpl" types="' . $type . '"}';
                    }
                }
            }
        }
