...
class rlListings
{
    ...
    {
        global $_response, $lang, $account_info, $rlDb;

        if (defined('IS_LOGIN') || (defined('REALM') && REALM == 'admin')) {
            $field = $GLOBALS['rlValid']->xSql($field);
            $value = $GLOBALS['rlValid']->xSql($value);

            $info = $rlDb->fetch(array('ID', 'Account_ID'), array($field => $value), null, 1, 'listings', 'row');

            if ($info['Account_ID'] == $account_info['ID'] || defined('REALM')) {
                unlink(RL_FILES . $value);
                $rlDb->query("UPDATE `{db_prefix}listings` SET `{$field}` = '' WHERE `ID` = '{$info['ID']}' LIMIT 1");
            } else {
                return $_response;
            }
        }
